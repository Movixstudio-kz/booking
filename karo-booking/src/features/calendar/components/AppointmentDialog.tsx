...
type AppointmentDialogProps = {
  ...
  onStatusChange: (appointment: BookingRecord, status: BookingStatus) => Promise<boolean>;
  onSave: (appointment: BookingRecord, isEditing: boolean) => Promise<CreateBookingResult>;
};
...
export function AppointmentDialog({ mode, appointment, defaultDate, staff, services, canEdit, canDelete, canChangeStatus, onClose, onEdit, onDelete, onStatusChange, onSave }: AppointmentDialogProps) {
  ...
  async function changeStatus(status: BookingStatus) {
    ...
    const changed = await onStatusChange(appointment, status);
    if (!changed) {
      setError(deniedMessage);
      return;
    }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await onSave(record, mode === "edit");
    if (!result.ok) {
      const messages = {
