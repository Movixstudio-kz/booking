...
export function AppointmentDialog({ mode, appointment, defaultDate, staff, services, onClose, onEdit, onDelete, onSave }: AppointmentDialogProps) {
  ...
  const availableStaff = useMemo(
    () => staff.filter((member) => member.serviceIds.includes(form.serviceId)),
    [form.serviceId, staff],
  );
  const status = appointmentStatuses.find((item) => item.value === appointment?.status);
  ...
  function changeService(serviceId: string) {
    ...
    setForm((current) => ({
      ...current,
      serviceId,
      staffId: matchingStaff.some((member) => member.id === current.staffId) ? current.staffId : (matchingStaff[0]?.id ?? ""),
    }));
    setError("");
  }
