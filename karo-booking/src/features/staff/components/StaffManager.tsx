...
export function StaffManager() {
  const { currentUser, staff, services, repositoryError, createStaff, updateStaff, archiveStaff } = useStaffRepository();
  ...
  async function submitForm(event: FormEvent<HTMLFormElement>) {
    ...
    const record = { ...form, name: form.name.trim(), position: form.position.trim(), phone: form.phone.trim() };
    const { id, ...input } = record;
    const result = id ? await updateStaff(id, input) : await createStaff(input);
    if (!result.ok) {
