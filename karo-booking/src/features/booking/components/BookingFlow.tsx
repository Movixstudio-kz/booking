...
export function BookingFlow({
  ...
  const [staffId, setStaffId] = useState(
    () => defaultStaff.find((member) => member.publicSlug === lockedStaffSlug)?.id ?? "",
  );
  ...
  const allActiveCatalogStaff = catalogStaff.filter((member) => member.isActive);
  const lockedStaff = lockedStaffSlug
    ? allActiveCatalogStaff.find((member) => member.publicSlug === lockedStaffSlug)
    : undefined;
  const lockedStaffId = lockedStaff?.id ?? "";
  const effectiveStaffId = lockedStaffId || staffId;
  const activeCatalogStaff = lockedStaffSlug
    ? allActiveCatalogStaff.filter((member) => member.id === lockedStaffId)
    : allActiveCatalogStaff;
  const activeCatalogServices = catalogServices.filter(
    (service) =>
      service.isActive &&
      (!lockedStaffSlug || lockedStaff?.serviceIds.includes(service.id)),
  );
  ...
  const selectedStaff = activeCatalogStaff.find(
    (member) => member.id === effectiveStaffId,
  );
  ...
  const refreshRepositoryData = useCallback(async () => {
    const [servicesResult, staffResult, organizationResult] = await Promise.all([
      loadPublicServices(),
      loadPublicStaff(),
      organizationSlug ? loadPublicOrganization(organizationSlug) : Promise.resolve(null),
    ]);
    ...
    else if (organizationResult && !organizationResult.ok) {
      setError("Салон недоступен для онлайн-записи.");
    }
  }, [organizationSlug]);

  async function refreshAvailableSlots() {
    if (!effectiveStaffId || !date || !selectedServiceDuration) {
  ...
  }, [date, effectiveStaffId, refreshRepositoryData, selectedBufferAfter, selectedBufferBefore, selectedServiceDuration]);
  ...
  function selectStaff(id: string) {
    if (lockedStaffSlug && id !== lockedStaffId) return;
    setStaffId(id);
    setDate("");
    setTime("");
    setError("");
  ...
  if (confirmedBooking) {
    const ConfirmationRoot = embedded ? "div" : "main";
    return <ConfirmationRoot className="min-h-screen bg-[#f6f8f1] px-5 py-8 sm:py-14"><BookingConfirmation booking={confirmedBooking} onCreateAnother={resetFlow} formatDate={formatDate} formatPrice={formatPrice} /></ConfirmationRoot>;
  }

  const Root = embedded ? "div" : "main";

  return (
    <Root className="min-h-screen bg-[#f6f8f1]">
      {!embedded && <header className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8"><Logo /><Link href={routes.home} className="text-sm font-semibold text-[#607068] transition hover:text-[#10231d]">Вернуться на главную</Link></header>}

      <div className="mx-auto max-w-7xl px-5 pb-16 pt-5 sm:px-8 sm:pt-10">
        <div className="max-w-2xl"><p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#619276]">Онлайн-запись</p><h1 className="mt-3 text-4xl font-semibold tracking-[-0.06em] text-[#10231d] sm:text-5xl">{lockedStaff ? `Запись к мастеру ${lockedStaff.name}` : "Выберите удобное время"}</h1><p className="mt-4 text-[#607068]">{lockedStaff ? "Выберите услугу мастера, дату и удобное свободное окно." : "Запись займёт не больше пары минут. Начните с выбора услуги."}</p></div>
        ...
        <div className="mt-9 grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px] lg:items-start">
          <section className="rounded-[28px] border border-[#dfe5da] bg-white p-5 shadow-xl shadow-[#315343]/5 sm:p-8">
            ...
            <div className="mt-9">
              ...
              {step === 2 && <div><h2 className="text-2xl font-semibold tracking-[-0.04em] text-[#10231d]">{lockedStaff ? "Ваш специалист" : "Выберите специалиста"}</h2><p className="mt-2 text-sm text-[#718178]">{lockedStaff ? "Мастер выбран по персональной ссылке и закреплён за этой записью." : "Показываем только сотрудников, которые выполняют выбранную услугу."}</p><div className="mt-6 grid gap-3 sm:grid-cols-2">{availableStaff.map((member) => <button type="button" key={member.id} onClick={() => selectStaff(member.id)} disabled={Boolean(lockedStaff)} aria-pressed={member.id === effectiveStaffId} className={`flex items-center gap-4 rounded-2xl border p-4 text-left transition ${member.id === effectiveStaffId ? "border-[#3dbf74] bg-[#e9faef] ring-2 ring-[#3ee58c]/20" : "border-[#dfe5da] hover:border-[#a9b8ac]"}`}><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-[#dff7e8] font-bold text-[#237347]">{member.name.slice(0, 1)}</span><span><span className="block font-semibold text-[#10231d]">{member.name}</span><span className="mt-1 block text-xs text-[#718178]">{lockedStaff ? "Выбрана по персональной ссылке" : "Доступна для записи"}</span></span></button>)}</div></div>}
