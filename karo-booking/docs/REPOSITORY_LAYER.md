...
Сохранённый в записи `durationMinutes` является историческим snapshot на момент создания записи. Изменение длительности услуги не перезаписывает уже существующие записи и не запускает автоматическую миграцию; fallback из каталога применяется только когда поле отсутствует в legacy-записи.
